...
void main() async {
  Future<DiskStore> newStore(
    ...
    return Directory.systemTemp
        .createTemp('stash_disk')
        .then((d) => DiskStore(d.path, fromEncodable: fromEncodable));
  }

  Future<DiskStore> newObjectStore(
      {dynamic Function(Map<String, dynamic>) fromEncodable}) {
    return newStore(fromEncodable: fromEncodable);
  }

  Future<DiskStore> newSampleObjectStore() {
    return newObjectStore(
        fromEncodable: (Map<String, dynamic> json) =>
            SampleClass.fromJson(json));
  }

  Future<void> deleteStore(DiskStore store) {
    return store.deleteAll();
  }

  test('Boolean', () async {
    await testStoreWith<DiskStore>(newStore, BoolGenerator(), deleteStore);
    await testCacheWith<DiskStore>(
        newStore, newDefaultCache, BoolGenerator(), deleteStore);
  });

  test('Int', () async {
    await testStoreWith<DiskStore>(newStore, IntGenerator(), deleteStore);
    await testCacheWith<DiskStore>(
        newStore, newDefaultCache, IntGenerator(), deleteStore);
  });

  test('Double', () async {
    await testStoreWith<DiskStore>(newStore, DoubleGenerator(), deleteStore);
    await testCacheWith<DiskStore>(
        newStore, newDefaultCache, DoubleGenerator(), deleteStore);
  });

  test('String', () async {
    await testStoreWith<DiskStore>(newStore, StringGenerator(), deleteStore);
    await testCacheWith<DiskStore>(
  ...
  test('List<bool>', () async {
    ...
    await testCacheWith<DiskStore>(newStore, newDefaultCache,
  ...
  test('Map<bool,double>', () async {
    await testStoreWith<DiskStore>(newStore,
  ...
  test('Class<bool>', () async {
    await testStoreWith<DiskStore>(newSampleObjectStore,
        SampleClassGenerator(BoolGenerator()), deleteStore);
    await testCacheWith<DiskStore>(newSampleObjectStore, newDefaultCache,
        SampleClassGenerator(BoolGenerator()), deleteStore);
  });
